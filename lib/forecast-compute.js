...
function computeSnapshot(deals, referenceDate, manualStore, probManual) {
  ...
  OverallCore.config({ MONTHS, ForecastEngine: FE, calcReceita, dealId: _dealId, stageProbDefault: STAGE_PROB_DEFAULT, stageProbSaved: {}, funnelProb: null, bidProb: 0.005, probManual: probManual || null });
...
function dealContributions(deals, referenceDate, manualStore, probManual) {
  FaturamentoManual.setData(manualStore || {});
  FaturamentoManual.config({ dealId: _dealId });
  FE.config({ MONTHS, getVpv, parseRevenueDate, addMonths, todayStr: () => referenceDate, calcReceita, monthLabels: MONTH_LABELS, referenceDate });
  OverallCore.config({ MONTHS, ForecastEngine: FE, calcReceita, dealId: _dealId, stageProbDefault: STAGE_PROB_DEFAULT, stageProbSaved: {}, funnelProb: null, bidProb: 0.005, probManual: probManual || null });
