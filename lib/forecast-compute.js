...
function mapFotoDeal(r) {
  ...
  return {
    ...
    arr_estimado: (function () {
      // Cascata de ARR — espelha api/forecast-table.js (docs/forecast-revenue-rules.md
      // §2b): 1) ARR Estimado; 2) 1ª Fatura × 12; 3) fallback vidas×VPV nas etapas
      // Diagnóstico/Cotação/Consultoria/Negociação. POC entra normalmente nos passos 1 e 2.
      const a = _num(r['ARR Estimado (R$)']); if (a != null && a > 0) return a;
      const pf = _num(r['1ª Fatura (R$)']); if (pf != null && pf > 0) return pf * 12;
      // POC sem ARR nem 1ª Fatura fica sem ARR (decisão 2026-08-11) — não estima via
      // vidas×VPV quando não há nenhum dado real de faturamento. Espelha forecast-table.js.
