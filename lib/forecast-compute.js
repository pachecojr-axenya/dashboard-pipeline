...
function mapFotoDeal(r) {
  ...
  const isPoc = _bool(r['É POC?']);
  // Cascata de ARR + origem — espelha api/forecast-table.js (docs/forecast-revenue-rules.md
  // §2b): 1) ARR Estimado (source 'hubspot'); 2) 1ª Fatura × 12 (source 'fatura'); 3)
  // fallback vidas×VPV nas etapas Diagnóstico/Cotação/Consultoria/Negociação (source 'vpv').
  // POC entra normalmente nos passos 1 e 2; sem nenhum dos dois, fica sem ARR (2026-08-11).
  const _arr = (function () {
    const a = _num(r['ARR Estimado (R$)']); if (a != null && a > 0) return { value: a, source: 'hubspot' };
    const pf = _num(r['1ª Fatura (R$)']); if (pf != null && pf > 0) return { value: pf * 12, source: 'fatura' };
  ...
  return {
    ...
    arr_estimado: _arr.value,
    arr_source: _arr.source,
    ...
    is_poc: isPoc,   // fotos a partir de 2026-07-13 capturam É POC? — alimenta o fallback vidas×VPV do ARR (acima) e o fallback do dealMonthly (forecast-engine.js, decisão 2026-08-11)
  };
}
...
// stageProb (5º arg, Fase 2): régua de prob por etapa a usar NO LUGAR da atual —
// vem do config sidecar da foto (Δ convicção). Omitido → régua vigente (composição).
function computeSnapshot(deals, referenceDate, manualStore, probManual, stageProb) {
  ...
  OverallCore.config({ MONTHS, ForecastEngine: FE, calcReceita, dealId: _dealId, stageProbDefault: stageProb || STAGE_PROB_DEFAULT, stageProbSaved: {}, funnelProb: null, bidProb: 0.005, probManual: probManual || null });
...
// Ganho OU Implantação — decisão do dono 2026-08-02: Implantação conta como Ganho
// no Delta, mesmo conjunto de CLOSED_STAGES usado no escopo do painel), 'avancou'
// (etapa posterior — sair por avanço ≠ sair por perda, requisito do dono
// 2026-07-24), 'saiu' (Perdido, regressão ou fora do pipe).
function _classifySaiu(stageA, rawDestino) {
  const destino = _stageBucket(rawDestino || 'Fora do pipe');
  if (CLOSED_STAGES.has(destino)) return 'ganho';
...
// Fechado (Tarefa do dono, reunião 2026-07-31; ajuste 2026-08-02: Implantação
// conta como Ganho): soma ARR/ARR Ponderado/receita (na FOTO A, mesmo critério do
// D05 "valor = ARR estimado na foto A") dos deals que SAÍRAM do conjunto escopado
// A→B porque foram para GANHO OU IMPLANTAÇÃO no período — mesma classificação D05
// (_classifySaiu) já usada no drill, aqui agregada para o card.
// Motivo do pedido: hoje um deal que vira Ganho entre A e B "derruba" o Total B do
// waterfall como se fosse perda de valor, quando na verdade é uma vitória (o valor
// só migrou de pipe aberto ponderado para fechado/contratado). "Total B + Fechado"
// comunica o quadro completo: o que já foi executado + o que ainda está por vir.
// ADITIVO/INFORMATIVO por decisão de design: NÃO entra na soma Σ Δ(linha) ==
// Δtotal do waterfall — esse invariante mede a variação do PIPE ABERTO ponderado
// (linhas do Overall), e Ganho já sai desse conjunto por definição (não tem linha
// própria no waterfall); somar Fechado ali mudaria a semântica do invariante
// existente em vez de só anotá-lo ao lado (ver scripts/test-delta-invariant.js).
// 4º argumento (2026-08-14, achado do dono: deal "Grupo Recovery - Vitalício", criado E ganho
// no mesmo dia, ~7min de diferença — nenhuma foto diária (02:59) o capturou aberto). Sem isso,
// um deal que nasce e fecha entre duas fotos é invisível no Delta inteiro: nunca aparece no
// escopo aberto (Ganho não é etapa selecionável) NEM aqui em cima (o loop de contribA só
// enxerga quem JÁ estava no conjunto aberto na Foto A — um deal que não existia em NENHUM
// estado na Foto A não passa por ele). `newAlreadyClosed` é a contribuição (mesmo formato de
// dealContributions, calculada em api/history.js sobre a Foto B) dos deals que não existem em
// NENHUM estado na Foto A (raw, não só o escopo aberto) e já são Ganho/Implantação na Foto B —
// "nasceram fechados". Somados incondicionalmente, sem checar escopo: um deal que nunca existiu
// não tem etapa-de-origem pra comparar contra a seleção de Escopo do usuário.
function closedWonAgg(contribA, contribB, rawBStageById, newAlreadyClosed) {
  const rawB = rawBStageById || {};
  const idsB = new Set((contribB || []).map(x => x.id));
  const out = { deals: 0, arr: 0, arrPond: 0, real12: 0, prob12: 0, realTotal: 0, probTotal: 0 };
  (contribA || []).forEach(a => {
    if (idsB.has(a.id)) return;   // continuou no escopo aberto — não fechou
    if (_classifySaiu(a.stage, rawB[a.id]) !== 'ganho') return;
    out.deals++;
    out.arr += a.arr || 0; out.arrPond += a.arrPond || 0;
    out.real12 += a.real12 || 0; out.prob12 += a.prob12 || 0;
    out.realTotal += a.realTotal || 0; out.probTotal += a.probTotal || 0;
  });
  (newAlreadyClosed || []).forEach(n => {
    out.deals++;
    out.arr += n.arr || 0; out.arrPond += n.arrPond || 0;
    out.real12 += n.real12 || 0; out.prob12 += n.prob12 || 0;
    out.realTotal += n.realTotal || 0; out.probTotal += n.probTotal || 0;
  });
  return out;
}
...
function dealContributions(deals, referenceDate, manualStore, probManual, stageProb) {
  FaturamentoManual.setData(manualStore || {});
  FaturamentoManual.config({ dealId: _dealId });
  FE.config({ MONTHS, getVpv, parseRevenueDate, addMonths, todayStr: () => referenceDate, calcReceita, monthLabels: MONTH_LABELS, referenceDate });
  OverallCore.config({ MONTHS, ForecastEngine: FE, calcReceita, dealId: _dealId, stageProbDefault: stageProb || STAGE_PROB_DEFAULT, stageProbSaved: {}, funnelProb: null, bidProb: 0.005, probManual: probManual || null });
...
// Estágios fechados (pós-venda) — usado no toggle do forecast-delta E em
// _classifySaiu/closedWonAgg (Implantação conta como Ganho no Delta, decisão do
// dono 2026-08-02).
...
// ── Escopo do /forecast-delta (2026-07-20; multiselect por etapa 2026-08-13) ───
// Remove SEMPRE pipeline Bid e etapa Standby. `scope` aceita 2 formas:
//   lista '|'-delimitada de etapas (ex. 'Diagnóstico|Cotação|Consultoria|Negociação') —
//     o multiselect do header manda exatamente isso, sempre, a cada request (mesmo
//     vazio: string vazia = 0 etapas selecionadas = 0 deals no escopo, seleção legítima,
//     não um "sem filtro"). Nomes fora de DELTA_ALL_OPEN_STAGES são ignorados
//     silenciosamente (defensivo contra param malformado).
//   'tudo' — sentinela LEGADO (pré-2026-08-06), mantido só pra quem ainda força esse
//     valor de propósito (D09: sempre "Tudo", independente do que o usuário selecionou
//     no header — ver api/history.js). Única forma de trazer Ganho/Implantação como
//     etapas do funil aqui; a lista '|'-delimitada NUNCA inclui Ganho/Implantação (só
//     as 5 etapas abertas) — esses dois continuam de fora do multiselect de propósito,
//     representados só pela barra aditiva "Fechado" do D02 (incluí-los ali duplicaria
//     com o Fechado). Régua de probabilidade de cada etapa é sempre a forecast_flat de
//     sempre (Reunião Agendada 6%, Diagnóstico 6% fixo, Cotação/Consultoria/Negociação
//     pela régua) — o escopo só decide QUAIS deals entram, nunca muda a fórmula.
//   ausente/null (nenhum dos dois) — usa DELTA_DEFAULT_STAGES (equivalente ao antigo
//     "Ativos"); só deve acontecer com um consumidor legado que não manda `scope` algum.
const DELTA_ALL_OPEN_STAGES = ['Reunião Agendada', 'Diagnóstico', 'Cotação', 'Consultoria', 'Negociação'];
const DELTA_DEFAULT_STAGES = ['Cotação', 'Consultoria', 'Negociação'];
// null = sentinela 'tudo'/ausente, tratada à parte por cada chamador; array (possivelmente
// vazio) = seleção explícita do multiselect.
function _parseDeltaScopeStages(scope) {
  if (scope == null || scope === 'tudo') return null;
  const set = new Set(String(scope).split('|').map(s => s.trim()).filter(Boolean));
  return DELTA_ALL_OPEN_STAGES.filter(s => set.has(s));
}
function applyDeltaScope(deals, scope) {
  if (scope === 'tudo') {
    return (deals || []).filter(d => d.pipeline !== 'Bid' && _stageBucket(d.stage) !== 'Stand by');
  }
  const parsed = _parseDeltaScopeStages(scope);
  const stages = parsed != null ? parsed : DELTA_DEFAULT_STAGES;
...
function deltaScopeStages(scope) {
  if (scope === 'tudo') return DELTA_ALL_OPEN_STAGES.concat(['Ganho', 'Implantação']);
  const parsed = _parseDeltaScopeStages(scope);
  return parsed != null ? parsed : DELTA_DEFAULT_STAGES;
}
// Uma linha do waterfall (OVERALL_ROWS) está no escopo? Descarta Bid sempre; fora de
// 'tudo', mantém só as linhas cujas etapas estão todas no conjunto selecionado.
function deltaRowInScope(row, scope) {
  ...
  return (row.stages || []).length > 0 && (row.stages || []).every(s => set.has(s));
}
...
// rawAStageById (2026-08-14, achado do dono): "Novo" aqui só quer dizer "não estava no
// conjunto FILTRADO por este stage/rowKey/etc. na Foto A" — pode ser um deal genuinamente
// novo OU um deal que já existia, só que em OUTRA etapa (avançou pra cá, ou raramente
// regrediu). Sem rawAStageById (raw, fora do filtro), o modal da etapa DESTINO nunca
// mostrava de onde o deal veio — só o lado da ORIGEM sabia o destino (via rawBStageById).
function drillGeneric(contribA, contribB, target, measure, rawBStageById, fieldOverride, rawAStageById) {
  const rawB = rawBStageById || {};
  const rawA = rawAStageById || {};
  ...
  const rows = Object.keys(ids).map(id => {
    ...
    const stageB = tipo === 'saiu' ? (rawB[id] || 'Fora do pipe') : (b ? b.stage : null);
    if (tipo === 'saiu') tipo = _classifySaiu(a.stage, rawB[id]);
    // "Novo" nesta VISÃO (filtrada por stage/rowKey/etc.) não significa "novo no pipe" — o
    // deal pode já existir na Foto A em OUTRA etapa. rawA (raw, sem filtro) resolve isso:
    // se achar a etapa bruta de origem, mostra de onde veio e reclassifica com a MESMA
    // _classifySaiu usada do lado da origem (destino aqui é sempre etapa aberta, nunca
    // Ganho/Perdido, porque veio de `b`, que já passou pelo filtro do escopo/etapa).
    let stageA = a ? a.stage : null;
    if (tipo === 'novo' && b) {
      const prevStage = rawA[id];
      if (prevStage) { stageA = prevStage; tipo = _classifySaiu(prevStage, b.stage); }
    }
    return {
      id, dealname: (b || a).dealname, stageA: stageA, stageB: stageB,
...
// Decomposição do D02 (waterfall) em causas SAUDÁVEIS (avançou de etapa, foi p/
// Ganho/Implantação, deal novo entrando, ou permaneceu e GANHOU valor) vs PERDA
// REAL (foi p/ Perdido/regrediu/saiu do pipe sem avançar, ou permaneceu e PERDEU
// valor) — pedido do dono (2026-08-07): "quero ver quanto da queda foi perda de
// fato vs. avanço de etapa". Mesma classificação do drill (_classifySaiu),
// agregada por linha do Overall (rowKey) e por TODAS as medidas de uma vez (sem
// round-trip por linha, ao contrário do drill on-demand). saudavel[m] +
// perdaReal[m] == Σ(B,rowKey,m) − Σ(A,rowKey,m) sempre, por construção (as duas
// somas particionam o mesmo conjunto de deals). Decisão de design do dono:
// Ganho/Implantação entra em "saudável" (fechar negócio é o melhor resultado
// possível, não uma perda).
const _HEALTH_MEASURES = ['real12', 'prob12', 'realTotal', 'probTotal', 'arr', 'arrPond'];
function healthByRow(contribA, contribB, rawBStageById) {
  const rawB = rawBStageById || {};
  const byKeyA = {}, byKeyB = {};
  (contribA || []).forEach(x => { if (!x.rowKey) return; (byKeyA[x.rowKey] = byKeyA[x.rowKey] || []).push(x); });
  (contribB || []).forEach(x => { if (!x.rowKey) return; (byKeyB[x.rowKey] = byKeyB[x.rowKey] || []).push(x); });
  const keys = {}; Object.keys(byKeyA).forEach(k => keys[k] = 1); Object.keys(byKeyB).forEach(k => keys[k] = 1);
  const blank = () => { const o = {}; _HEALTH_MEASURES.forEach(m => o[m] = 0); return o; };
  const out = {};
  Object.keys(keys).forEach(key => {
    const inA = byKeyA[key] || [], inB = byKeyB[key] || [];
    const mapA = {}; inA.forEach(x => mapA[x.id] = x);
    const mapB = {}; inB.forEach(x => mapB[x.id] = x);
    const ids = {}; inA.forEach(x => ids[x.id] = 1); inB.forEach(x => ids[x.id] = 1);
    const saudavel = blank(), perdaReal = blank();
    Object.keys(ids).forEach(id => {
      const a = mapA[id], b = mapB[id];
      if (!a) { _HEALTH_MEASURES.forEach(m => saudavel[m] += b[m] || 0); return; }   // novo: entrou na linha
      if (!b) {   // saiu da linha — classifica pelo destino bruto em B
        const tipo = _classifySaiu(a.stage, rawB[id]);
        const bucket = tipo === 'saiu' ? perdaReal : saudavel;   // 'saiu' = Perdido/regrediu/sumiu (drill chama de "Caiu (Perdido)")
        _HEALTH_MEASURES.forEach(m => bucket[m] += -(a[m] || 0));
        return;
      }
      // permaneceu na linha — valor pode ter subido (saudável) ou caído (perda real)
      _HEALTH_MEASURES.forEach(m => { const d = (b[m] || 0) - (a[m] || 0); if (d >= 0) saudavel[m] += d; else perdaReal[m] += d; });
    });
    out[key] = { saudavel, perdaReal };
  });
  return out;
}

// Decomposição do D02 em 5 fatias — novo / avançou / permaneceu / perdido /
// probabilidade (pedido do dono, 2026-08-17: sucessora de healthByRow, que só
// tinha 2 baldes). Só cobre 'arr'/'arrPond' (as únicas medidas que o D02 usa de
// fato — horizon fica fixo em 'arr' desde 2026-08-06, ver forecast-delta.html).
// Fórmula da fatia Probabilidade — pra cada deal que ENTROU numa linha vindo de
// OUTRA etapa entre A e B (avançou): prob_A = arrPond_A/arr_A na etapa de ORIGEM,
// prob_B = arrPond_B/arr_B na etapa de destino (esta linha).
//   efeito_probabilidade = arr_A × (prob_B − prob_A)   → pipe "inflado" pela régua
//   efeito_avançou        = arr_A×prob_A (valor relocado) + (arr_B−arr_A)×prob_B
// Soma dos dois = arrPond_B exato (a fatia Probabilidade é o que ficava escondido
// dentro de "avançou" no healthByRow). Em 'arr' (Real) não tem o que decompor —
// avançou entra com o valor cheio, sem probabilidade (sempre 0).
// saída (a linha PERDE o deal): não decompõe — nada pra "reclassificar" numa etapa
// que o deal está deixando; vai inteiro pra 'perdido' (saiu) ou 'avancou' (foi
// pra Ganho/Implantação ou etapa posterior — mesma filosofia do healthByRow: sair
// por avanço/fechamento é resultado bom, não perda).
// Quando um deal "novo" na linha (não estava nela em A) TAMBÉM não existe em
// NENHUMA etapa da Foto A (raw, via rawAStageById) → fatia 'novo' de verdade. Se
// existia em outra etapa mas essa etapa não tem contribuição própria em contribA
// (fora do escopo, sem rowKey) → não dá pra saber o ARR/probabilidade de origem;
// conta como 'avançou' sem split, mesma filosofia do caveat já documentado pro
// destino de deal Bid no D05 (não inventar precisão que a fonte não tem).
function blank5() { return { novo: 0, avancou: 0, permaneceu: 0, perdido: 0, probabilidade: 0 }; }
function movementSlices(contribA, contribB, rawAStageById, rawBStageById) {
  const rawA = rawAStageById || {}, rawB = rawBStageById || {};
  const byKeyA = {}, byKeyB = {}, byIdA = {};
  (contribA || []).forEach(x => { if (x.rowKey) (byKeyA[x.rowKey] = byKeyA[x.rowKey] || []).push(x); byIdA[x.id] = x; });
  (contribB || []).forEach(x => { if (x.rowKey) (byKeyB[x.rowKey] = byKeyB[x.rowKey] || []).push(x); });
  const keys = {}; Object.keys(byKeyA).forEach(k => keys[k] = 1); Object.keys(byKeyB).forEach(k => keys[k] = 1);
  const out = {};
  Object.keys(keys).forEach(key => {
    const inA = byKeyA[key] || [], inB = byKeyB[key] || [];
    ...
    const mapB = {}; inB.forEach(x => mapB[x.id] = x);
    const ids = {}; inA.forEach(x => ids[x.id] = 1); inB.forEach(x => ids[x.id] = 1);
    const arr = blank5(), arrPond = blank5();
    Object.keys(ids).forEach(id => {
      const a = mapA[id], b = mapB[id];
      if (!a && b) {
        const origin = byIdA[id];
        if (origin) {
          const probA = origin.arr ? origin.arrPond / origin.arr : 0;
          const probB = b.arr ? b.arrPond / b.arr : 0;
          arr.avancou += b.arr || 0;   // Real: valor cheio, sem split (não tem probabilidade a reclassificar)
          arrPond.avancou += (origin.arrPond || 0) + ((b.arr || 0) - (origin.arr || 0)) * probB;
          arrPond.probabilidade += (origin.arr || 0) * (probB - probA);
        } else if (rawA[id] != null) {
          arr.avancou += b.arr || 0; arrPond.avancou += b.arrPond || 0;
        } else {
          arr.novo += b.arr || 0; arrPond.novo += b.arrPond || 0;
        }
        return;
      }
      if (a && !b) {
        const isLoss = _classifySaiu(a.stage, rawB[id]) === 'saiu';
        const bucket = isLoss ? 'perdido' : 'avancou';
        arr[bucket] += -(a.arr || 0); arrPond[bucket] += -(a.arrPond || 0);
        return;
      }
      if (a && b) {
        arr.permaneceu += (b.arr || 0) - (a.arr || 0);
        arrPond.permaneceu += (b.arrPond || 0) - (a.arrPond || 0);
      }
    });
    out[key] = { arr, arrPond };
  });
...
module.exports = {
  ...
  dealDiff, closedWonAgg, refMonthIndex, dealId: _dealId, healthByRow, movementSlices
};
