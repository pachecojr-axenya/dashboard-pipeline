...
// ── Escopo do /forecast-delta (2026-07-20; extensão 2026-08-13) ─────────────
// Remove SEMPRE pipeline Bid e etapa Standby. Três modos:
//   'ativos' (padrão do painel) = só Cotação/Consultoria/Negociação (Diagnóstico NÃO
//              entra em Ativos — decisão do dono 2026-07-20; só aparece em Pipe/Tudo).
//   'pipe'   = "Todo o Pipe" (toggle reintroduzido 2026-08-13, pedido do dono): Ativos +
//              Reunião Agendada + Diagnóstico. Ganho/Implantação NÃO entram aqui de
//              propósito — continuam representados só pela barra aditiva "Fechado" do
//              D02 (evita a etapa Ganho/Implantação aparecer DUAS vezes: como barra
//              própria do waterfall E como Fechado). Régua de probabilidade das duas
//              etapas novas é a mesma forecast_flat de sempre (Reunião Agendada 6%,
//              Diagnóstico 6% fixo) — nenhum número novo, só deixa de filtrar os deals.
//   'tudo'   = legado (pré-2026-08-06): todas as demais de Vendas (Reunião Agendada +
//              Diagnóstico + as 3 + Ganho + Implantação). Mantido só para consumidores
//              existentes (ex.: D09 força scope=tudo de propósito); a UI não expõe mais
//              esta opção — o toggle do header usa 'ativos'/'pipe'.
const DELTA_ACTIVE_STAGES = new Set(['Cotação', 'Consultoria', 'Negociação']);
const DELTA_PIPE_STAGES = new Set(['Reunião Agendada', 'Diagnóstico', 'Cotação', 'Consultoria', 'Negociação']);
function applyDeltaScope(deals, scope) {
  return (deals || []).filter(d => {
    ...
    if (scope === 'pipe') return DELTA_PIPE_STAGES.has(st);
    return DELTA_ACTIVE_STAGES.has(st);
  });
}
// Lista ordenada de etapas exibidas no funil, por escopo (sem Bid/Standby/Perdido).
function deltaScopeStages(scope) {
  if (scope === 'tudo') return ['Reunião Agendada', 'Diagnóstico', 'Cotação', 'Consultoria', 'Negociação', 'Ganho', 'Implantação'];
  if (scope === 'pipe') return ['Reunião Agendada', 'Diagnóstico', 'Cotação', 'Consultoria', 'Negociação'];
  return ['Cotação', 'Consultoria', 'Negociação'];
}
// Uma linha do waterfall (OVERALL_ROWS) está no escopo? Descarta Bid sempre; em
// 'ativos'/'pipe' mantém só as linhas cujas etapas estão todas no conjunto do escopo.
function deltaRowInScope(row, scope) {
  if (row.isBid) return false;
  if (scope === 'tudo') return true;
  const set = scope === 'pipe' ? DELTA_PIPE_STAGES : DELTA_ACTIVE_STAGES;
