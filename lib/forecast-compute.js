...
//   'ativos' (padrão do painel) = só Diagnóstico/Cotação/Consultoria/Negociação.
//   'tudo'   = todas as demais de Vendas (Reunião Agendada + as 4 + Ganho +
//              Implantação). Perdido já sai no scopeDeals.
const DELTA_ACTIVE_STAGES = new Set(['Diagnóstico', 'Cotação', 'Consultoria', 'Negociação']);
...
function stageUnified(contribA, contribB, rawBStageById) {
  ...
  return _STAGE_LABELS.map(s => {
    ...
    // Calcular movimento: novos (entrou), caiuPerdido (saiu para Perdido), 
    // saiuGanhoIgnorado (saiu para Ganho — não conta como caiu)
    ...
    let novos = 0, caiuPerdido = 0, saiuGanhoIgnorado = 0;
    
    // Novos: está em B mas não em A nesta etapa
    idsB.forEach(id => { if (!idsA.has(id)) novos++; });
    
    // Saiu: está em A mas não em B nesta etapa — checar destino
    idsA.forEach(id => {
      if (!idsB.has(id)) {
        const destino = _stageBucket(rawB[id] || 'Fora do pipe');
        if (destino === 'Perdido') caiuPerdido++;
        else if (destino === 'Ganho') saiuGanhoIgnorado++;
      }
    });
    
    return { 
      stage: s, a, b, delta, 
      movement: { novos, caiuPerdido, saiuGanhoIgnorado } 
    };
  });
}
...
function drillRow(contribA, contribB, rowKey, measure, rawBStageById) {
  ...
  const rows = Object.keys(ids).map(id => {
    ...
    // Quando saiu, distinguir "ganho" de "saiu"
    if (tipo === 'saiu') {
      const destino = _stageBucket(rawB[id] || 'Fora do pipe');
      if (destino === 'Ganho') tipo = 'ganho';
      else if (destino === 'Perdido') tipo = 'saiu';
    }
...
function drillGeneric(contribA, contribB, target, measure, rawBStageById, fieldOverride) {
  ...
  const rows = Object.keys(ids).map(id => {
    ...
    let tipo = !a ? 'novo' : (!b ? 'saiu' : 'permaneceu');
    const stageB = tipo === 'saiu' ? (rawB[id] || 'Fora do pipe') : (b ? b.stage : null);
    // Quando saiu, distinguir "ganho" (foi para Ganho) de "saiu" (foi para Perdido/outro)
    if (tipo === 'saiu') {
      const destino = _stageBucket(rawB[id] || 'Fora do pipe');
      if (destino === 'Ganho') tipo = 'ganho';
      else if (destino === 'Perdido') tipo = 'saiu';
