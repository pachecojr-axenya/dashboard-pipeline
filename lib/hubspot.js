...
async function fetchCotacaoTickets(token) {
  ...
  const properties = [
    ...
    'source_type', 'hs_ticket_category', 'comercial_vidas',
    'time_to_first_agent_reply', 'cotacao_status_final',
  ...
  for (let i = 0; i < allCompanyIds.length; i += 100) {
    ...
    try {
      const compResp = await hubspotPost(token, '/crm/v3/objects/companies/batch/read', {
        properties: ['name', 'hs_object_id', 'segmento', 'beneficio_axenya', 'operadora_atual'],
        inputs: batch.map(id => ({ id: String(id) }))
      });
      (compResp.results || []).forEach(r => {
        companyNames[r.id] = r.properties?.name || 'Unknown';
        companySegmento[r.id] = r.properties?.segmento || null;
        companyBeneficio[r.id] = r.properties?.beneficio_axenya || null;
        companyOperadora[r.id] = r.properties?.operadora_atual || null;
      });
    } catch (e) {
  ...
  allResults.forEach(t => {
    ...
    t._companyName = primaryId ? (companyNames[primaryId] || 'Unknown') : null;
    t._companySegmento = primaryId ? companySegmento[primaryId] : null;
    t._companyBeneficio = primaryId ? companyBeneficio[primaryId] : null;
    t._companyOperadora = primaryId ? companyOperadora[primaryId] : null;
  });
