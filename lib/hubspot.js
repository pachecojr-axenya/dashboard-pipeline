...
const STAGE_IDS = Object.keys(STAGE_MAP);
...
async function hubspotPost(token, endpoint, body) {
  ...
  if (res.status === 401 || res.status === 403) throw new Error('HubSpot: autenticação falhou. Verifique o token.');
...
async function fetchAllDeals(token) {
  ...
  while (hasMore) {
    const body = {
      filterGroups: [{ filters: [{ propertyName: 'pipeline', operator: 'EQ', value: HUBSPOT_PIPELINE }] }],
      properties,
      limit: 200,
      after
    };
...
async function fetchCotacaoTickets(token) {
  const COTACAO_PIPELINE = '847948895';
  const stageIds = ['1262941018','1262941019','1293302338','1262941020','1263003034','1263003035','1263003036','1263003037','1263003038','1263003039'];
...
module.exports = {
  HUBSPOT_PIPELINE, STAGE_MAP, STAGE_IDS, AE_NAMES,
