...
// ===== CONTROLE DE ACESSO =====
// Regra principal: QUALQUER e-mail do domínio axenya.com tem acesso ao dashboard.
// Além disso, e-mails EXTERNOS específicos podem ser liberados via:
//   - a lista AUTHORIZED_EMAILS abaixo (no código), ou
//   - a env var ALLOWED_EMAILS (separada por vírgula), sem editar o código:
//     ALLOWED_EMAILS=parceiro@empresa.com,consultor@outra.com
// (e-mails @axenya.com não precisam estar em lista nenhuma.)
const AUTHORIZED_EMAILS = [
  // 'parceiro@empresa.com',   // externos autorizados (fora do domínio axenya.com)
];
...
// Único ponto de decisão de acesso:
//   1) todo e-mail @axenya.com é autorizado (match EXATO de domínio, não "endsWith");
//   2) externos só se constarem no allowlist (arquivo + ALLOWED_EMAILS).
function isEmailAuthorized(email) {
  const e = (email || '').toLowerCase().trim();
  if (!e || e.indexOf('@') === -1) return false;
  const domain = e.slice(e.lastIndexOf('@') + 1);
  if (domain === ALLOWED_DOMAIN) return true;
  return getAllowedEmails().indexOf(e) !== -1;
}
