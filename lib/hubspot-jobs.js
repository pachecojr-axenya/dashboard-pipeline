...
/**
 * Execuções dos DOIS jobs da plataforma, mais recentes primeiro.
 *
 * O `close` entra aqui desde 13/08/2026, e a razão é concreta: a trava de
 * concorrência olhava só o `reconcile`, então o `close` era INVISÍVEL para ela e
 * o botão Atualizar podia disparar por cima de um fechamento em andamento. Os
 * dois rodam `CREATE OR REPLACE TABLE` no mesmo gold — duas execuções
 * simultâneas reescrevem as mesmas tabelas, e quem lê no meio vê o estado de
 * ninguém.
 *
 * Passava despercebido porque o `close` rodava uma vez por dia às 20:30, fora do
 * horário de quem clica no botão. Com refresh intraday em horário comercial a
 * colisão deixa de ser hipótese.
 *
 * Falha de um dos dois não derruba a leitura: a trava com a lista do outro ainda
 * é melhor que trava nenhuma.
 */
async function listReconcileExecutions(token, pageSize = 10) {
  const listas = await Promise.all([JOB_RECONCILE, JOB_CLOSE].map(async (job) => {
    try {
      const data = await gapi(token, 'GET', `${RUN_BASE}/jobs/${job}/executions?pageSize=${pageSize}`);
      return (data.executions || []).map(summarizeExecution).map((e) => Object.assign(e, { job }));
    } catch (e) {
      return [];
    }
  }));
