 ...
 * JavaScript.
 *
 * Duas decisões de PARIDADE, deliberadas:
 *
 * 1. A etapa é atribuída ao pipeline ATUAL do deal (`dim_deal.pipeline_id`), não
 *    ao pipeline registrado na entrada. Deal migra de pipeline (armadilha A3:
 *    783509571 → 782758156 → 803749153 num deal real), e usar o pipeline da
 *    entrada seria mais correto — mas a versão antiga usava o atual, e o objetivo
 *    aqui é poder comparar. Trocar isso é mudança de definição, e tem de ser
 *    decidida como tal.
 * 2. `owner_changes` é `COUNT(*) - 1`, e o -1 não é chute: a contagem de
...
async function funnelStages(pipelineIds, since, until) {
  ...
  const sql = `
    ...
    entradas AS (
      ...
      SELECT e.object_id AS deal_id, e.stage_id,
    ...
    duracoes AS (
      SELECT e.object_id AS deal_id, e.stage_id, SUM(e.days_in_stage) AS dias
      FROM ${wh.t('silver', 'fact_stage_entry')} e
      JOIN deals d ON d.deal_id = e.object_id
    ...
    SELECT
      ...
      ARRAY(SELECT AS STRUCT deal_id, deal_name, pipeline_id,
            ...
            FROM deals) AS deals,
      ARRAY(SELECT AS STRUCT deal_id, stage_id, entered_date FROM entradas) AS entradas,
      ARRAY(SELECT AS STRUCT deal_id, stage_id, dias FROM duracoes) AS duracoes,
  ...
  return {
    ...
    entradas: (r.entradas || []).map((e) => ({
      deal: wh.str(e.deal_id), stage: wh.str(e.stage_id), data: wh.str(e.entered_date),
    })),
    duracoes: (r.duracoes || []).map((d) => ({
      deal: wh.str(d.deal_id), stage: wh.str(d.stage_id), dias: Number(d.dias),
    })),
