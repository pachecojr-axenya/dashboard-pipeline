...
async function companyDeals(companyId) {
  ...
  return rows.map((r) => ({
    ...
    hs_is_closed_won: String(wh.bool(r.is_won_cur)),
    hs_is_closed_lost: String(wh.bool(r.is_lost_cur)),
    sdr: wh.str(r.sdr),
...
async function cotacaoTickets() {
  const sql = `
    WITH t AS (
      SELECT d.ticket_id, d.subject, d.category, d.source_type, d.source_detail,
             ...
             JSON_VALUE(r.payload, '$.time_to_first_agent_reply') AS time_to_first_agent_reply,
             JSON_VALUE(r.payload, '$.cotacao_status_final') AS cotacao_status_final
  ...
  // Nomes de empresa + segmento/benefício/operadora atual: uma consulta só,
  // para todos os ids vistos. As 3 últimas não são coluna do dim_company (só
  // propriedade custom) — vêm do raw_company via JSON_VALUE, mesmo padrão já
  // usado pra source_label em workloadPayload().
  const todosIds = [...new Set(rows.flatMap((r) => r.company_ids || []).filter(Boolean).map(String))];
  const companyNames = {}, companySegmento = {}, companyBeneficio = {}, companyOperadora = {};
  if (todosIds.length) {
    const lista = todosIds.filter((id) => /^\d+$/.test(id)).map((id) => `'${id}'`).join(',');
    if (lista) {
      const { rows: cn } = await wh.query(`
        SELECT c.company_id, c.company_name,
               JSON_VALUE(r.payload, '$.segmento') AS segmento,
               JSON_VALUE(r.payload, '$.beneficio_axenya') AS beneficio_axenya,
               JSON_VALUE(r.payload, '$.operadora_atual') AS operadora_atual
        FROM ${wh.t('silver', 'dim_company')} c
        LEFT JOIN ${wh.t('bronze', 'raw_company')} r ON r.object_id = c.company_id
        WHERE c.is_current AND c.company_id IN (${lista})
      `);
      cn.forEach((c) => {
        const id = String(c.company_id);
        companyNames[id] = wh.str(c.company_name) || 'Unknown';
        companySegmento[id] = wh.str(c.segmento) || null;
        companyBeneficio[id] = wh.str(c.beneficio_axenya) || null;
        companyOperadora[id] = wh.str(c.operadora_atual) || null;
      });
    }
  }
  ...
  const tickets = rows.map((r) => {
    ...
    const primaryId = compIds.length
      ? (compIds.find((id) => companyNames[id] && companyNames[id] !== 'Unknown') || compIds[0])
      : null;
    ...
    const t = {
      ...
      time_to_first_agent_reply: wh.str(r.time_to_first_agent_reply),
      cotacao_status_final: wh.str(r.cotacao_status_final),
      ...
      _companyName: primaryId ? (companyNames[primaryId] || 'Unknown') : null,
      _companySegmento: primaryId ? companySegmento[primaryId] : null,
      _companyBeneficio: primaryId ? companyBeneficio[primaryId] : null,
      _companyOperadora: primaryId ? companyOperadora[primaryId] : null,
      stage_label: wh.str(r.stage_label),
      ownerName: wh.str(r.owner_name) || wh.str(r.owner_id) || '-',
    };
...
/**
 * Payload nominal do Workload de BDR: empresas criadas, contatos criados,
 * transições de `hs_lead_status` e toques, na janela. Substitui, no
 * /api/bdr-workload, três buscas paginadas + histórico em lote + 3 rodadas de
 * associação + a busca separada do Treble — por request.
 *
 * DECISÕES DE FIDELIDADE, todas verificáveis com `?fonte=api`:
 *
 * · **Treble não precisa de passo próprio.** Na API, WhatsApp via integração
 *   (app 26063081) chega com `hubspot_owner_id` NULO e por isso não vem no fetch
 *   por dono; o endpoint faz uma segunda busca e atribui pelo dono do CONTATO
 *   associado. No armazém isso já está resolvido na fato: `owner_attributed`
 *   marca o toque cuja atribuição veio do contato, e `owner_id` já é o dono
 *   efetivo. O `source_label = 'INTEGRATION'` continua distinguindo Treble de
 *   WhatsApp digitado à mão.
 * · **`desfechoId` (GUID) viaja junto do rótulo.** O `bdr-workload-semantic`
 *   classifica "conectada" pelo GUID, e trocar a chave mudaria a definição entre
 *   hoje e o histórico. O rótulo é só exibição e depende de idioma/customização.
 * · **`fonte` do objeto** (Apollo/Lusha/API interna/Manual) é derivada no
 *   chamador, do mesmo `source_detail`/`source_label` — a regra é do consumidor.
 */
async function workloadPayload(teamIds, sinceMs, untilMs) {
  if (!teamIds.length) return { empresas: [], contatos: [], transicoes: [], toques: [] };
  const lista = teamIds.filter((id) => /^\d+$/.test(String(id))).map((id) => `'${id}'`).join(',');
  if (!lista) return { empresas: [], contatos: [], transicoes: [], toques: [] };

  const P = [
    { name: 'since', type: 'INT64', value: String(sinceMs) },
    { name: 'until', type: 'INT64', value: String(untilMs) },
  ];
  const janela = 'BETWEEN TIMESTAMP_MILLIS(@since) AND TIMESTAMP_MILLIS(@until)';

  // 1. Empresas criadas na janela por dono do time. `hs_created_by_user_id` só
  // existe nas manuais, então a atribuição é pelo dono — push de Apollo/Lusha
  // conta como inserção do BDR, igual à versão antiga.
  const empresas = wh.query(`
    SELECT c.company_id, c.company_name, c.owner_id, c.employees,
           c.source_detail, c.hs_created_at,
           JSON_VALUE(r.payload, '$.hs_object_source_label') AS source_label
    FROM ${wh.t('silver', 'dim_company')} c
    LEFT JOIN ${wh.t('bronze', 'raw_company')} r ON r.object_id = c.company_id
    WHERE c.is_current AND c.owner_id IN (${lista})
      AND c.hs_created_at ${janela}
  `, P);

  // 2. Contatos criados na janela por dono do time — COM ou SEM hs_lead_status
  // (inserção não depende de status; o /api/bdr-leads é que só cobre quem tem).
  const contatos = wh.query(`
    SELECT ct.contact_id, ct.first_name, ct.last_name, ct.job_title, ct.owner_id,
           ct.company_id_prop, ct.lead_status, ct.numero_de_colaboradores,
           ct.source_detail, ct.hs_created_at,
           JSON_VALUE(r.payload, '$.hs_object_source_label') AS source_label,
           cp.company_name, cp.employees
    FROM ${wh.t('silver', 'dim_contact')} ct
    LEFT JOIN ${wh.t('bronze', 'raw_contact')} r ON r.object_id = ct.contact_id
    LEFT JOIN ${wh.t('silver', 'dim_company')} cp
      ON cp.company_id = ct.company_id_prop AND cp.is_current
    WHERE ct.is_current AND ct.owner_id IN (${lista})
      AND ct.hs_created_at ${janela}
  `, P);

  // 3. Transições de hs_lead_status DENTRO da janela. `old_value` dá o "de" sem
  // reconstruir a série no JavaScript — e ele já vem colapsado, então re-save do
  // mesmo status não vira transição.
  const transicoes = wh.query(`
    SELECT ch.object_id AS contact_id, ch.old_value, ch.new_value, ch.changed_at,
           ct.first_name, ct.last_name, ct.job_title, ct.owner_id,
           ct.company_id_prop, cp.company_name, cp.employees
    FROM ${wh.t('silver', 'fact_crm_change')} ch
    JOIN ${wh.t('silver', 'dim_contact')} ct
      ON ct.contact_id = ch.object_id AND ct.is_current
    LEFT JOIN ${wh.t('silver', 'dim_company')} cp
      ON cp.company_id = ct.company_id_prop AND cp.is_current
    WHERE ch.object_type = 'contact' AND ch.property = 'hs_lead_status'
      AND ch.changed_at ${janela}
      AND ct.owner_id IN (${lista})
  `, P);

  // 4. Toques do time na janela. Treble incluído sem passo extra — ver o bloco
  // de decisões acima.
  const toques = wh.query(`
    SELECT e.engagement_id, e.kind, e.owner_id, e.owner_attributed, e.occurred_at,
           e.duration_ms, e.disposition_id, e.disposition_label, e.direction,
           e.channel_type, e.source_label, e.company_id, e.contact_id, e.deal_id
    FROM ${wh.t('silver', 'fact_engagement')} e
    WHERE e.owner_id IN (${lista})
      AND e.occurred_at ${janela}
      AND e.kind IN ('calls','emails','communications','notes','tasks','meetings')
  `, P);

  const [E, C, T, A] = await Promise.all([empresas, contatos, transicoes, toques]);
  return {
    empresas: E.rows, contatos: C.rows, transicoes: T.rows, toques: A.rows,
  };
}
...
module.exports = {
  ...
  activities, companyDeals, dealProbHistory, cotacaoTickets, ownerMap, bdrCalls, bdrLeadContacts, funnelStages, workloadPayload,
};
