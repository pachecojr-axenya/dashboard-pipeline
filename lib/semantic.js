...
// Tickets da Cotação. ticketStageIds() inclui as 3 etapas removidas do portal
// (removida_do_portal: true) de propósito: o 1.0 ainda consulta as props
// hs_date_entered/exited delas — paridade até a limpeza consciente.
function ticketPipelineId() {
  return referencia.tickets_cotacao.pipeline.id;
}
function ticketStageIds() {
  return referencia.tickets_cotacao.etapas.map(e => e.id);
}

module.exports = { referencia, PIPELINES, pipelineLabels, stageMap, activeStageIds, lostStageIds, stageProb, ticketPipelineId, ticketStageIds };
