...
// BUG (achado 2026-08-02, relatado ao vivo na reunião de forecast: deal Perdido
// aparecia como "avançou" no drill do waterfall): o comparador antigo (`a.timestamp
// < b.timestamp ? 1 : -1`) devolve -1 também quando os timestamps são IGUAIS —
// viola contrato de ordem total do Array.prototype.sort (não-transitivo em
// empates). Para a maioria dos deals isso não aparece porque as transições têm
// timestamps distintos, mas quando o HubSpot grava DUAS mudanças de dealstage no
// MESMO instante (workflow em cadeia, ex.: Negociação → Perdido processado no
// mesmo request), o sort podia embaralhar um array corretamente ordenado
// (mais-recente-primeiro, contrato documentado do HubSpot) e valueAt() devolvia a
// etapa INTERMEDIÁRIA em vez do estado FINAL (Perdido) — reproduzido em
// scripts/test-snapshot-history.js. Fix: comparador numérico próprio (0 em
// empate) → sort ESTÁVEL, preserva a ordem que o HubSpot já entrega para os
// empates em vez de arriscar reordená-los errado.
function valueAt(versions, cutoff) {
  if (!versions || !versions.length) return null;
  const sorted = versions.slice().sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
