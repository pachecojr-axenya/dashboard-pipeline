...
async function getAccessToken() {
  ...
  // BOM/whitespace defensivo: env var criada via pipe do PowerShell pode vir com U+FEFF
...
async function apiReq(token, method, path, body, spreadsheetId) {
  const sid = spreadsheetId || SPREADSHEET_ID;
  const res = await fetch(
    `https://sheets.googleapis.com/v4/spreadsheets/${sid}${path}`,
    {
      method,
...
async function getSheetTitles(token, spreadsheetId) {
  const data = await apiReq(token, 'GET', '?fields=sheets.properties.title', null, spreadsheetId);
...
// Lê um range A1 em uma planilha arbitrária. Mantém o cliente zero-deps e evita
// duplicar JWT em rotas que precisam ler outra planilha operacional.
async function readSpreadsheetRange(spreadsheetId, rangeA1) {
  if (!spreadsheetId) throw new Error('spreadsheetId obrigatório');
  const token = await getAccessToken();
  const data  = await apiReq(token, 'GET', `/values/${encodeURIComponent(rangeA1)}`, null, spreadsheetId);
  return data.values || [];
}

async function listSpreadsheetTabs(spreadsheetId) {
  if (!spreadsheetId) throw new Error('spreadsheetId obrigatório');
  const token = await getAccessToken();
  return getSheetTitles(token, spreadsheetId);
}
...
module.exports = { appendDailyRow, writeMonthlySnapshot, listMonthlyTabs, readSnapshot, listTabs, readRange, readSpreadsheetRange, listSpreadsheetTabs, appendRow };
