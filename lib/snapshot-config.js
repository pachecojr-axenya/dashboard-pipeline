...
// Backfill EMBUTIDO no bundle (lib/snapshot-config-backfill.json, gerado por
// scripts/backfill-snapshot-config.js --export e versionado no git): config
// reconstruída das fotos anteriores à Fase 2. Fallback DEPOIS do BQ — um sidecar
// capturado ao vivo sempre vence a reconstrução.
let _backfillMap;
function _backfillLoad(date) {
  if (_backfillMap === undefined) {
    try { _backfillMap = require('./snapshot-config-backfill.json'); }
    catch (e) { _backfillMap = null; }
  }
  const v = _backfillMap && _backfillMap[date];
  return v ? Object.assign({ origin: 'backfill' }, v) : null;
}

// Lê o sidecar da data: BQ (ao vivo) → backfill embutido → /tmp (dev). null =
// foto sem config nenhuma — o chamador aplica a config ATUAL com flag visível.
async function load(date) {
  try {
    if (bq.isConfigured()) {
      try { const v = await _bqLoad(date); if (v) return Object.assign({ origin: 'live' }, v); }
      catch (e) { /* BQ indisponível/sem permissão → tenta os fallbacks */ }
    }
    const bf = _backfillLoad(date);
    if (bf) return bf;
    const t = _tmpLoad(date);
    return t ? Object.assign({ origin: t.source && String(t.source).indexOf('backfill') === 0 ? 'backfill' : 'live' }, t) : null;
  } catch (e) {
