...
function currentStageProb() {
  try {
    ...
    const v = ref && ref.reguas && ref.reguas.forecast_flat && ref.reguas.forecast_flat.valores;
...
async function save(date, source) {
  try {
    ...
    if (bq.isConfigured()) return await _bqSave(date, cfg);
...
module.exports = { save, load, currentConfig, currentStageProb, TABLE_CONFIG };
