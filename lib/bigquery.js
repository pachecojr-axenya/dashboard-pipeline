...
async function insertSnapshotRows(snapshotDate, snapshotType, dealRows, capturedAt, table) {
...
async function deriveWeeklyFromDaily(snapshotDate, snapshotType) {
  ...
  const r = await insertSnapshotRows(snapshotDate, snapshotType || 'semanal', rows.slice(1), null, TABLE_WEEKLY);
  return r;
}
...
module.exports = {
  ...
  query, listSnapshotDates, hasSnapshot, readSnapshotRows,
};
