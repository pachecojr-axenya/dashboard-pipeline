...
async function insertSnapshotRows(snapshotDate, snapshotType, dealRows, capturedAt, table, header) {
  ...
  let total = 0;
  // insertId inclui capturedAt: retry de REDE do mesmo insertAll (mesmo capturedAt)
  // continua deduplicado; uma RECAPTURA (replace=today, capturedAt novo) não é
  // engolida pela janela de dedup do streaming. Reexecucao do cron no mesmo dia
  // segue barrada ANTES do insert pelo guard de contagem (api/snapshot.js).
  const capKey = String(cap).replace(/[^0-9]/g, '').slice(0, 14);
  for (let i = 0; i < dealRows.length; i += 500) {
    const chunk = dealRows.slice(i, i + 500);
    const rows = chunk.map((r) => ({
      insertId: snapshotDate + ':' + capKey + ':' + (r[idIdx] == null ? 'na' : String(r[idIdx])),
      json: toObj(r),
    }));
