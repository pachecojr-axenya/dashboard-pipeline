...
// Mapa label->bucket do desfecho de ligacao no caminho LIVE. O /api/bdr-workload
// resolve `activity.desfecho` para o LABEL PT-BR via /calls/v1/dispositions
// (ex.: 'Conectado', 'Sem resposta'), enquanto o BQ guarda o GUID. Os dois
// caminhos precisam concordar, senao o total de hoje muda de definicao no meio
// da serie. Politica: sucesso = 'connected' apenas; voicemail e bucket proprio.
const LIVE_CALL_OUTCOME = { 'conectado': 'connected', 'sem resposta': 'no_answer', 'ocupado': 'busy', 'numero errado': 'wrong_number', 'número errado': 'wrong_number', 'deixou mensagem de voz': 'voicemail', 'deixou mensagem ativa': 'voicemail' };
function callOutcomeOf(activity) { const raw = String(activity && activity.desfecho || '').trim().toLowerCase(); if (!raw) return null; return LIVE_CALL_OUTCOME[raw] || null; }
