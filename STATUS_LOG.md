...
> Front-only em `public/forecast.html` (função `exportData`), usando dados já presentes no DOM/payload. **NÃO commitado e NÃO deployado (só local).** Motivo: ao exportar (CSV/XLSX), as 48 colunas mensais saíam com rótulos duplicados (`JAN 2026 … DEZ 2027` duas vezes) porque o **header de grupo** ("Receita Real"/"Receita Probabilizada") não é exportado — o arquivo não deixava claro qual bloco era Real e qual era Probabilizada.
