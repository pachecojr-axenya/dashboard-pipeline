...
- **PR/merge:** commit direto na main `b6a1fc4` (pushado antes do deploy; preflight exige HEAD==origin/main).
- **Deploy canônico:** `dpl_8QGy9Vuhhozjs151cTZruHmTKFaF` (READY, target production), aliases `https://axenya-pipeline-dashboard.vercel.app` e `https://project-bsmfu.vercel.app`. Pós-deploy: HTML serve `bdr-workload-v2.js?v=7` + `charts.js?v=2`, sem script v1; `/bdr-workload.js` (v1) = 404.
- **Auth Vercel (aprendizado):** todos os secrets Vercel do GCP são da conta pessoal `salencar-lang` e NÃO têm o team `axenya-f1a041f6`; deploy exigiu `npx vercel login` (device flow) + `--scope axenya-f1a041f6`. Sem git auto-deploy neste projeto.
