...
## 0. Fonte única de receita (SSOT) — vale para TODO o dashboard

> **Diretriz canônica:** esta regra é a **Regra primária nº 3** do `STATUS_LOG.md`
> (Diretrizes do Projeto). Se houver divergência, o STATUS_LOG manda. O resumo
> abaixo existe só para dar contexto ao build do coverage.

> **Regra que precede tudo.** O update de forecast estabeleceu **duas bases
> canônicas de receita**, e **todo gráfico, KPI ou painel que envolva receita, em
> qualquer tela do dashboard, deriva delas.** Nenhum painel recalcula receita por
> conta própria.
>
> 1. **Previsão real** — o valor mensal de fato dos deals em Ganho/Implantação que
>    já faturam, vindo do faturamento manual (`api/faturamento-manual.js`, Upstash
>    KV; fonte única `_fcDealMonthly`), sem cutoff de `createdate`.
> 2. **Previsão probabilizada** — a régua de receita por modelo de cobrança
>    ponderada pela probabilidade de etapa puxada ao vivo do funil (C06).
>
> **Implicações:**
> - O Pipeline Coverage é um **consumidor** dessas bases, não uma fonte paralela.
> - Qualquer novo gráfico de receita (por AE, por segmento, por produto, ARR
>   bridge, board, etc.) deve puxar dessas duas séries. Se um painel diverge de
>   outro no número de receita, é bug de fonte, não de arredondamento.
> - O motor de régua compartilhado (§4) e a dedup de deals duplos (§7.1) fazem
>   parte dessa fonte: alterou a regra de receita, alterou para todos os painéis.
...
| 7 | **Realizado = valor real digitado** ✅ implementado (30/06) | Deals em Ganho/Implantação que já faturam usam o **valor mensal real** do faturamento manual (editor estilo planilha, persistido em Upstash KV), **sem cutoff de `createdate`** — não mais estimativa por fórmula. O gate é `vencimento_primeira_fatura` vencido. Reconciliação automática com o extrato de comissionamento segue v2. |
| 8 | **Probabilidade = funil dinâmico C06 (ao vivo)** ✅ implementado (30/06) | Probabilidade por etapa puxada **ao vivo do funil**, não tabela estática. Diagnóstico fixo em 6%. Override manual continua válido e deve afetar o coverage. |
...
| 10 | **Deals duplicados (dois modelos) contam uma vez** | Quando um cliente tem dois negócios simultâneos (fee por vida + corretagem), o forecast considera **só um**: o de **menor TCV** (base 12 meses) e **prazo de pagamento mais longo** (worst case). Regra da reunião de 12/06 (ver §7.1). Sem isso, o coverage infla por double-counting. |

> **Status.** As decisões 7 e 8 já foram implementadas no dashboard (commits de
> 30/06). Este documento passa a tratá-las como comportamento existente a
> reaproveitar, não a construir do zero. A decisão 10 (dedup) ainda **não** está
> implementada e é obrigatória na v1.
...
> **Nota sobre a Decisão 5 (cutoff).** O cutoff `createdate >= 2025-09-01` vale
> para o forecast de negócio novo. Duas exceções já existem no código: deals
> **BID** entram sem cutoff (todo o 2025) e a **receita realizada** (Ganho/Implantação
> faturando) também ignora o cutoff. O coverage deve respeitar essas mesmas
> exceções ao montar cada camada.
...
## 5. Probabilidade (C06) — já implementado

A probabilidade por etapa **calculada dinamicamente pelo funil (C06)** já está no ar
(puxada ao vivo do funil desde 30/06). O coverage **consome** essa probabilidade, não
recria. Definição (do próprio modal de regras, ~linha 750):
...
- **Diagnóstico** usa probabilidade fixa de **6%** (não vem do funil, definido em
  30/06). Ganho/Implantação = 100%.
...
Horizonte: mês atual + 5 (6 meses). **Antes de somar**, aplicar a deduplicação de
deals duplos (§7.1). Para cada mês do horizonte:

```
dealsElegiveis = dedupDeals(feed)          // §7.1: 1 deal por cliente/oportunidade

Para cada deal em dealsElegiveis:
...
## 7.1. Deduplicação de deals com dois modelos (Decisão 10)

**Origem:** reunião de forecast quinzenal de **12/06/2026** (Ivan + Cíntia). Quando
um mesmo cliente pode ser fechado por dois modelos (fee por vida **e** corretagem),
o HubSpot não tem mais a opção "ambos", então a prática é **criar dois negócios**
separados que correm juntos no funil. Regra de desfecho: **quando um fecha, o outro
morre.** Enquanto os dois estão abertos, o forecast contaria o mesmo cliente duas
vezes. Isso infla o coverage.

**Regra (falas literais da reunião):**

- Considerar **apenas um** dos dois deals no forecast.
- Critério de escolha, ambos mirando o *worst case*:
  1. **Menor TCV** (Total Contract Value em base de **12 meses**). Ivan: *"Eu
     pegaria o menor. Sempre menor... a gente sempre tá olhando em TCV, contrato de
     12 meses."*
  2. **Prazo de pagamento mais longo** (vigência/início de receita mais distante).
     Cíntia: *"sempre consideraria o mais longo de pagamento e o valor menor, essas
     duas condições."*
- O **TCV é só fator de decisão**, não precisa aparecer no dash. Cíntia: *"qual tem
  o menor TCV, vou nesse. E o output continua igualzinho. O TCV é só fator de
  decisão."* Ou seja, o horizonte e os valores exibidos não mudam; o TCV serve
  apenas para escolher qual deal entra.

**Implementação (o "gate de seleção" que o Ivan citou):**

```
dedupDeals(feed):
  agrupar deals por cliente/oportunidade (mesma empresa + par fee/corretagem)
  para cada grupo com 2+ modelos coexistindo:
    tcv12(deal) = soma de calcReceitaMes(n, deal).total para n = 1..12
    escolher o deal com MENOR tcv12
    em empate de tcv12: escolher o de data_prevista_para_receita MAIS TARDIA
    descartar os demais do grupo (não entram na soma do §7)
  grupos com um único modelo passam intactos
```

**A definir (não bloqueia):** a chave de agrupamento exata (como identificar que
dois deals são "o mesmo cliente/oportunidade" — provável companhia associada +
flag de modelo). Confirmar qual propriedade do HubSpot amarra o par. Deixar a chave
isolada para ajuste fácil.
...
4. **Realizado vs projetado (Decisão 7):** deals em Ganho/Implantação com
   `vencimento_primeira_fatura` já vencido usam o **valor real** do faturamento
   manual (fonte única `_fcDealMonthly`), não a régua estimada. Distinguir
   visualmente "realizado" de "projetado" (ex.: cor/opacidade), com tooltip
   avisando que o realizado é o valor digitado, ainda não reconciliado com o
   extrato de comissionamento (v2).
...
- **Campos por deal necessários** (confirmar que o feed que o `dashboard.html`
  ...
  `data_prevista_para_receita`, `createdate`, `vencimento_primeira_fatura`.
- **Faturamento manual (realizado):** os valores reais mês a mês vivem no
  `api/faturamento-manual.js` (Upstash KV) e são a fonte única de `_fcDealMonthly`
  para deals já faturando. O coverage deve ler esses valores para o realizado, não
  recalcular pela régua. Ver Decisão 7.
...
- [ ] A probabilidade vem do funil C06 (ao vivo); editar override muda o coverage;
      "Restaurar padrão do funil" volta ao C06. Diagnóstico = 6%.
- [ ] Cliente com dois deals (fee por vida + corretagem) entra **uma vez** no
      coverage, pelo de menor TCV 12 meses (empate → início de receita mais tardio).
- [ ] Deal em Ganho/Implantação já faturando usa o **valor real** do faturamento
      manual, não a régua estimada, e é sinalizado como realizado.
...
2. Implementar `dedupDeals(feed)` (§7.1) e aplicar antes de qualquer soma.
3. No `dashboard.html`, montar a série mensal de 6 meses, ponderando por C06,
   usando o valor real do faturamento manual para os deals já faturando.
4. Conectar a meta do campo existente, `÷12`.
5. Calcular os dois coverages por mês.
6. Renderizar KPIs + barras empilhadas (recorrente sólido, pontual hachurado) +
   linha de meta + rótulo de coverage + distinção realizado/projetado.
7. Implementar o drill-down (memória de cálculo) e o indicador de completude.
8. Incluir Diagnóstico (6%) marcado como estimado, com a regra própria isolada (§10).
...
- **Reconciliação automática** do realizado com o extrato de comissionamento (o
  faturamento manual já entrega o realizado digitado na v1; falta cruzar com o
  extrato bancário de comissões).
